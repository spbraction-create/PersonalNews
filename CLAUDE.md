...
### 2. השלמת מקורות טורים 1/2/5 — ⏳ בעבודה, ניצחון חלקי (session 2026-08-13)
**מה נבדק:** 4 מועמדי פיד מ-mako (news-israel, news-money, weekend/magazine, tech12).
**התובנה המרכזית של הצעד הזה:** סטטוס 200 + RSS תקין מבנית **לא מספיק** בשביל לאשר מקור — חובה לבדוק תאריכי פרסום בפועל. 3 מתוך 4 פידי mako שנראו תקינים בבדיקה ראשונית התבררו כקפואים/פגומים כשבדקנו תאריכים:
- **news-israel.xml, news-money.xml** — RSS תקין, אבל הפריט העדכני ביותר מיולי 2026 (למעלה מחודש), גם עם cache-busting. קטגוריות נטושות, ככל הנראה מאז המעבר לתשתית N12.
- **weekend.xml** (מגזין) — כל הפריטים חוזרים עם אותו pubDate בדיוק (זמן הבקשה, לא זמן פרסום), בפורמט לא תקני. אי אפשר לסנן ל-24 שעות בכלל.
- **הזוכה היחיד: Nexter** (`rcs.mako.co.il/rss/cd0c4e8fc83b8310VgnVCM2000002a0c10acRCRD.xml`) — ערוץ הדיגיטל/טק הפעיל של mako, פיד חי ומעודכן להיום ממש. נוסף ל-`data/sources.json` עבור **טור 4 (טק)** בלבד.

**עדיין דחוף — לא נפתר:** טור 1 (חדשות) וטור 2 (כלכלה) **עדיין בלי מקור ישראלי חי בפועל**. mako נכשל לגמרי בשני הטורים האלה. כלכליסט ו-Ynet עדיין לא נבדקו — זו המשימה הכי דחופה שנשארת בסעיף הזה. sport5 (טור 5) גם עדיין לא נבדק. פירוט מלא ב-`SOURCES.md` (סעיף "אימות פידים" + הערות "מלכודת" בכל טור).
